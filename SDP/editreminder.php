...
if (isset($_POST['send'])) {
    ...
    $updateQuery = "UPDATE tblreminder SET detail='$detail', date='$datetime' WHERE id='$myIDS'";
    if (mysqli_query($connection, $updateQuery)) {
        echo 'Reminder successfully updated!';
...
<body>
    <div class="heading">
        <h3>Edit Reminder</h3>
        <p> <a href="userhome.php?myID=<?php echo $_SESSION['myID']; ?>">Home</a> / <a href="reminder.php?myID=<?php echo $_SESSION['myID']; ?>">Reminder</a> / Edit Reminder</p>
    </div>

    <?php
    $query = "SELECT * FROM tblreminder WHERE id = '$myIDS'";
    ...
    ?>
        <div class="contact">
            <form action="#" method="post" id="registerForm" style="font-size: 20px;border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.2);padding: 20px; animation: openAccount 0.5s ease forwards">
                Detail: <input type="text" name="detail" class="boxs" value="<?php echo $rows['detail']; ?>"> <br>
                Datetime: <input type="text" name="datetime" id="datetimepicker" class="boxss" value="<?php echo $rows['date']; ?>"> <br>
